...
if(!class_exists('GlobalSettings')) {
  class GlobalSettings {
    ...
    public function visibility_menu() {
      add_submenu_page(
        'options-general.php', 'Menu Visibility', 'Menu Visibility', 'manage_options', 'menu-visibility', array( $this, 'menu_visibility_page_callback')
      );
    }
    ...
    public function mv_field_menus_cb( $args ) {
      ...
      if (!empty($menus)) {
        foreach($menus as $menu) {
          $checked = (is_array($options) && in_array($menu->slug, array_keys($options))) ? 'checked' : '';
          ?>
          <div>
            <input type="checkbox" id="<?php echo $menu->slug ?>" <?php echo $checked ?> name="menu_visibility_menus[<?php echo $menu->slug?>]">
            <label for="<?php echo $menu->slug?>"> <?php esc_html_e( $menu->name, 'menu_visibility_menus' ); ?> </label>
          </div>
          <?php
        }
      }
    ...
    public function mv_field_ct_cb( $args ) {
      ...
      foreach(get_post_types() as $post_type) {
        ...
        $checked = (is_array($options) && in_array($post_object->name, array_keys($options))) ? 'checked' : '';
        if ($post_object->public) {
          ?>
          <div>
            <input type="checkbox" id="<?php echo $post_object->name ?>" <?php echo $checked ?> name="menu_visibility_post_types[<?php echo $post_object->name?>]">
            <label for="<?php echo $post_object->name?>"> <?php esc_html_e( $post_object->label, 'menu_visibility_post_types' ); ?> </label>
          </div>
          <?php
        }
      }
    ...
    public function mv_field_roles_cb( $args ) {
      ...
      foreach($wp_roles->roles as $role_id => $role) {
        $checked = (is_array($options) && in_array($role_id, array_keys($options))) ? 'checked' : '';
        ?>
        <div>
          <input type="checkbox" id="<?php echo $role_id ?>" <?php echo $checked ?> name="menu_visibility_user_roles[<?php echo $role_id?>]">
          <label for="<?php echo $role_id ?>"> <?php esc_html_e( $role['name'], 'menu_visibility_user_roles' ); ?> </label>
        </div>
        <?php
      }
    }
